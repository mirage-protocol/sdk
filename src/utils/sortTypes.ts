...
const convertToBytes = (str: string): number[] => {
  return [...str].map((char) => char.charCodeAt(0) & 0xff)
}

const compareByteVector = (left: number[], right: number[]): Result => {
  ...
  while (idx < left_length && idx < right_length) {
    ...
    if (left_byte < right_byte) return Result.SMALLER
    else if (left_byte > right_byte) return Result.GREATER

    idx++
  }

  if (left_length < right_length) return Result.SMALLER
  else if (left_length > right_length) return Result.GREATER
